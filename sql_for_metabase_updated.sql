...
--16. Monthly cost from purchasing from suppliers--
SELECT
...
--17. Display products that are reaching their expiration date--
SELECT
    p.product_name,
    po.product_id,
    po.order_id,
...
--18. Comparison of Monthly revenue percentage difference with the current month--
SELECT 
    year,
    month,
    total_sales_amount,
    FIRST_VALUE(total_sales_amount) OVER (ORDER BY year DESC, month DESC) AS most_recent,
	((FIRST_VALUE(total_sales_amount) OVER (ORDER BY year DESC, month DESC) - total_sales_amount)
     / FIRST_VALUE(total_sales_amount) OVER (ORDER BY year DESC, month DESC)) * 100 AS percentage_diff
FROM 
(
SELECT
